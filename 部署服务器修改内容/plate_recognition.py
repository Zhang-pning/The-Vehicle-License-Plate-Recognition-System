...
# ========== 修复：统一路径大小写 + 强制添加 YOLOv5 路径 ==========
current_dir = os.path.dirname(os.path.abspath(__file__))
# 关键：匹配实际目录名 YOLOv5（大写），而非 yolov5（小写）
yolov5_path = os.path.join(current_dir, 'YOLOv5')

# 强制添加到 Python 搜索路径的最前面
if yolov5_path not in sys.path:
    sys.path.insert(0, yolov5_path)
    print(f"已添加 YOLOv5 路径: {yolov5_path}")
else:
    print(f"YOLOv5 路径已存在: {yolov5_path}")

# 验证 utils 目录是否存在
utils_path = os.path.join(yolov5_path, 'utils')
if os.path.exists(utils_path):
    print(f"utils 目录存在: {utils_path}")
else:
    print(f"警告：utils 目录不存在: {utils_path}")
    sys.exit(1)

# ========== 导入 YOLOv5 模块（使用绝对导入） ==========
# 修复：从 YOLOv5 根目录导入，而非相对路径
from models.common import DetectMultiBackend  # 直接导入，因为已添加 YOLOv5 路径
